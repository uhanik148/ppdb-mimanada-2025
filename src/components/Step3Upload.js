	...
	if (submitSuccess) {
		return (
			<div style={{ textAlign: 'center', marginTop: '2rem' }}>
				<div style={{ color: '#4CAF50', fontSize: '60px', marginBottom: '1rem', display: 'flex', justifyContent: 'center' }}>✓</div>
				<h2 style={{ fontSize: '24px', marginBottom: '1rem' }}>Pendaftaran Berhasil!</h2>
				<p style={{ marginBottom: '2rem' }}>Terima kasih telah mendaftar. Tim kami akan meninjau pendaftaran Anda dan akan menghubungi Anda segera.</p>
				<button
		...
		>
			...
			{errorDetails && (
				<div style={{ backgroundColor: '#ffebee', padding: '1rem', borderRadius: '8px', border: '1px solid #ffcdd2', marginBottom: '1rem' }}>
					...
					<p>
						<strong>Pesan:</strong> {errorDetails.message}
					</p>
					{errorDetails.response && (
						<>
							<p>
								<strong>Status:</strong> {errorDetails.response.status}
							</p>
							{errorDetails.response.data && (
								<div>
									<p>
										<strong>Detail dari server:</strong>
									</p>
									<pre style={{ backgroundColor: '#f5f5f5', padding: '0.5rem', borderRadius: '4px', overflow: 'auto', maxHeight: '150px' }}>{JSON.stringify(errorDetails.response.data, null, 2)}</pre>
								</div>
							)}
						</>
					)}
			...
			<div style={infoBox}>
				...
				<p>
					Silakan upload <strong>satu file dokumen</strong> yang berisi semua persyaratan berikut:
				</p>
				...
				<p style={{ marginTop: '0.75rem' }}>Anda dapat menggunakan Microsoft Word, PDF, atau dokumen OneDrive yang berisi semua persyaratan di atas.</p>
			</div>
			...
			<div style={formGroup}>
				...
				/>
				<p style={{ color: '#666', fontSize: '12px', marginTop: '0.25rem' }}>Format yang diterima: .doc, .docx, .pdf (Maksimal 10MB)</p>
			</div>

			<div style={confirmationBox}>
				<h3 style={{ fontSize: '16px', marginBottom: '0.5rem' }}>Konfirmasi Pengiriman Data</h3>
				<p style={{ color: '#666', fontSize: '14px' }}>Dengan menekan tombol "Submit Pendaftaran", Anda menyetujui bahwa:</p>
