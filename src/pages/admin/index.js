      ...
      <div className="flex-1 bg-gray-100 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center text-sm">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl shadow p-4 text-center">
            <div className="text-2xl font-bold text-green-800">{pendaftarBaru.length}</div>
            <div className="text-sm text-gray-600">Jumlah Pendaftar</div>
          </div>
          <div className="bg-white rounded-xl shadow p-4 text-center">
            <div className="text-2xl font-bold text-green-800">
              {pendaftarBaru.filter(siswa => siswa.status === "Lengkap").length}
            </div>
            <div className="text-sm text-gray-600">Data Lengkap</div>
          </div>
          <div className="bg-white rounded-xl shadow p-4 text-center">
            <div className="text-2xl font-bold text-green-800">
              {pendaftarBaru.filter(siswa => siswa.status === "Belum Lengkap").length}
            </div>
        ...
        <div className="bg-white rounded-xl shadow p-4">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {pendaftarBaru.map((siswa, idx) => (
                  <tr key={idx} className="border-b">
                    <td className="p-2">{siswa.nama}</td>
                    <td className="p-2">{siswa.nisn}</td>
                    <td className="p-2">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${siswa.status === "Lengkap" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                        {siswa.status}
                      </span>
                    </td>
                    <td className="p-2">
                      <Link href={`/admin/pendaftar/${siswa.nisn}`}>
                        <button className="bg-green-700 hover:bg-green-800 text-white text-xs px-3 py-1 rounded">
                          Lihat Detail
                        </button>
                      </Link>
                    </td>
